...
class GameboardView extends World
  ...
  int verticalCells = 0;
  late List<ActorEntity> actors;
  late ActorEntity actorView;
  late ActorEntity walkingAreaView;
  ...
  void onLoad() {
    ...
    isBoardLoaded = false;
    actors = <ActorEntity>[]; // Initialize the list of cards
  ...
  void clearBoard() {
    ...
    for (ActorEntity card in actors) {
  ...
  void addActor() {
    ActorModel graveModel = ActorModel(
        x: 0, y: 0, distance: 0, status: ActorModel.RELIC, prev: null);
    actorView = ActorEntity(
        actorModel: graveModel, position: Vector2(0, 0), size: Vector2(32, 32));
  ...
  void addWalkingArea() {
    ActorModel walkingAreaModel = ActorModel(
        x: 0, y: 0, distance: 0, status: ActorModel.FLOOR, prev: null);
    walkingAreaView = ActorEntity(
        actorModel: walkingAreaModel,
        position: Vector2(160, 200),
        size: Vector2(200, 32));
  ...
  void addNPC() {
    ActorModel npcModel = ActorModel(
        x: 0, y: 0, distance: 0, status: ActorModel.OLD_SAILOR, prev: null);
