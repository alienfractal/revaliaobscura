...
class EnergyBar extends PositionComponent with HasGameRef<RevaliaObs> {
  ...
  EnergyBar({
    ...
    required Vector2 energyBarSize,
    ...
    energyBarBackground = RectangleComponent(
      ...
      size: size,
      paint: Paint()
        ..color = Color(0xffa13d3b), // Red background for the energy bar
    );
    ...
    energyBar = RectangleComponent(
      ...
      paint: Paint()
        ..color = const Color.fromARGB(
            255, 255, 255, 255), // Green fill for the energy bar
    );
  ...
  void updateEnergy(double newEnergy) {
    currentEnergy =
        newEnergy.clamp(0, maxEnergy); // Ensure it stays within bounds
